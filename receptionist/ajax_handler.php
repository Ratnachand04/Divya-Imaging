...
if (!empty($search_term)) {
    ...
    $sql .= " AND (b.id LIKE ? OR p.uid LIKE ? OR p.name LIKE ?) ORDER BY b.id DESC";
...
// Compute base URL once for link generation
$base_url = rtrim(str_replace(str_replace('\\', '/', $_SERVER['DOCUMENT_ROOT']), '', str_replace('\\', '/', dirname(__DIR__))), '/');
...
if ($result->num_rows > 0) {
    while ($bill = $result->fetch_assoc()) {
        ...
        $output .= '<td>' . htmlspecialchars($bill['patient_uid']) . '</td>';
    ...
    $output = '<tr><td colspan="9" style="text-align:center;">No results found.</td></tr>';
}
