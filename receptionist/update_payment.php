...
$stmt = $conn->prepare("SELECT b.*, p.name as patient_name FROM {$bills_source} JOIN {$patients_source} ON b.patient_id = p.id WHERE b.id = ? AND b.receptionist_id = ?");
...
$bill['amount_paid'] = round(max(0, (float)$bill['amount_paid']), 2);
$bill['balance_amount'] = calculate_pending_amount((float)$bill['net_amount'], (float)$bill['amount_paid']);
$bill['payment_status'] = derive_payment_status_from_amounts((float)$bill['net_amount'], (float)$bill['amount_paid'], 'Due');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $new_balance_amount = calculate_pending_amount((float)$bill['net_amount'], (float)$new_amount_paid);
        $new_payment_status = derive_payment_status_from_amounts((float)$bill['net_amount'], (float)$new_amount_paid, 'Due');
