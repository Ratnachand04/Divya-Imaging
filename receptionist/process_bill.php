...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        for ($attempt = 0; $attempt < 5; $attempt++) {
            ...
            $stmt_patient = $conn->prepare("INSERT INTO patients (uid, name, age, sex, address) VALUES (?, ?, ?, ?, ?)");
            if (!$stmt_patient) {
                throw new Exception("Patient preparation failed: " . $conn->error);
            }

            $stmt_patient->bind_param("ssiss", $uid, $patient_name, $patient_age, $patient_sex, $patient_address);
