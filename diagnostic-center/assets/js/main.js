document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- Live Input Guards (restrict invalid characters while typing) ---
    const applyLiveInputGuards = () => {
        const alphaIds = [
            'patient_name',
            'patient_city',
        ...
        alphaIds.forEach((id) => {
            ...
            input.addEventListener('input', () => {
                const next = normalizeAlpha(input.value);
                if (input.value !== next) {
                    input.value = next;
                }
            });
            ...
            input.addEventListener('blur', () => {
                input.value = normalizeAlpha(input.value).trim();
            });
        });
        ...
        phoneIds.forEach((id) => {
            ...
            const maxLenAttr = parseInt(input.getAttribute('maxlength'), 10);
            const maxLen = Number.isNaN(maxLenAttr) ? undefined : maxLenAttr;

            input.addEventListener('input', () => {
                const next = normalizeDigits(input.value, maxLen);
                if (input.value !== next) {
                    input.value = next;
                }
            });
