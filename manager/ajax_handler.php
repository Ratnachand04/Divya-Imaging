...
$bills_source_b2 = function_exists('table_scale_get_read_source') ? table_scale_get_read_source($conn, 'bills', 'b2') : '`bills` b2';
$bills_source_b3 = function_exists('table_scale_get_read_source') ? table_scale_get_read_source($conn, 'bills', 'b3') : '`bills` b3';
...
$test_packages_source_count = function_exists('table_scale_get_read_source') ? table_scale_get_read_source($conn, 'test_packages', 'tpc') : '`test_packages` tpc';
...
$stmt_kpis = $conn->prepare(
    "SELECT
        ...
        (
            ...
            JOIN {$bills_source_b2} ON bi.bill_id = b2.id
...
$packages_count_result = $conn->query("SELECT COUNT(*) AS total_packages, SUM(CASE WHEN tpc.status = 'active' THEN 1 ELSE 0 END) AS active_packages FROM {$test_packages_source_count}");
...
$stmt_referral = $conn->prepare(
        "SELECT b.referral_type, COUNT(b.id) as count
         FROM {$bills_source}
         WHERE b.bill_status != 'Void'
             AND DATE(b.created_at) BETWEEN ? AND ?
         GROUP BY b.referral_type
         ORDER BY count DESC"
);
...
$stmt_payment = $conn->prepare(
        "SELECT b.payment_mode, SUM(b.net_amount) as total
         FROM {$bills_source}
         WHERE b.bill_status != 'Void'
             AND DATE(b.created_at) BETWEEN ? AND ?
